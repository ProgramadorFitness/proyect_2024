...
const Table_Payments = ({ data }: Props) => {
    ...
    async function updateStateBlackList() {
        const api = new Api()
    ...
    const filteredData = data.filter((dato) => {
        const matchByState = dato.state?.toLowerCase().includes(search.toLocaleLowerCase())
            ...
            const searchMatch = matchById || matchById_number || matchByFq || matchByState;

            if (Object.values(checkboxState).every((value) => !value)) return searchMatch; // Mostrar todos si no hay checkboxes seleccionados
            return Object.entries(checkboxState).some(([key, value]) => value && dato?.state.toLowerCase() === key.toLowerCase()) && searchMatch;
          });
    ...
    return (
        <div className='shadow-md sm:rounded-lg'>
            <th scope="col" className=" flex justify-center items-center text-black bg-slate-300 hover:bg-v focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-slate-500 dark:hover:bg-slate-500 dark:focus:ring-slate-500">
                ...
                <div className='pl-3'>
                    <button onClick={updateStateBlackList}
                        className=" block text-black bg-slate-400 hover:bg-slate-400 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-slate-300 dark:hover:bg-slate-400 dark:focus:ring-slate-400" type="button"
                    ><GrDocumentUpdate  size={25} /></button>
                </div>
                ...
                <div className="flex">
                    {Object.entries(checkboxState).map(([key, value]) => (
                        <div key={key} className="flex items-center me-4">
                            ...
                            />
                            <label htmlFor={`${key}-checkbox`} className="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                                {key.toUpperCase()}
                            </label>
                        </div>
                    ))}
            ...
            <div className='overflow-auto bg-slate-300 flex items-center justify-between flex-column flex-wrap md:flex-row space-y-4 md:space-y-0 pb-4 bg-black dark:bg-slate-300'>
                <table id='miTabla' className="w-full text-sm text-left rtl:text-right text-black-500 dark:text-black-400">
                    ...
                    <tbody className="text-xs text-black-300 uppercase bg-slate-300 dark:bg-slate-200 dark:text-black-400">
                        {filteredData?.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).map((item) => (
                            <tr key={item.id} style={{ backgroundColor: item.state === 'expired pay' ? 'pink' : item.state === 'pay' ? 'lightgreen' : item.state === 'pay of part' ? 'lightyellow' : 'inherit'  }}>

                                <td className="border-b border-slate-400 px-6 py-4">{item.id}</td>
                                <td className="border-b border-slate-400 px-6 py-4">{item.id_loan}</td>
                                ...
                                <td className="border-b border-slate-400 px-6 py-4">{item.duesPaid}</td>
                                <td className="border-b border-slate-400 px-6 py-4">{item.duesValue?.toLocaleString('es-CO', { style: 'currency', currency: 'COP' })}</td>
                                <td className="border-b border-slate-400 px-6 py-4">{item.duesRealPay?.toLocaleString('es-CO', { style: 'currency', currency: 'COP' })}</td>
                                <td className="border-b border-slate-400 px-6 py-4">{item.outBalance?.toLocaleString('es-CO', { style: 'currency', currency: 'COP' })}</td>
                                <td className="border-b border-slate-400 px-6 py-4">{item.realDatePay?.substring(0, 10)}</td>
                                <td className="border-b border-slate-400 px-6 py-4">{item.datePay?.substring(0, 10)}</td>
