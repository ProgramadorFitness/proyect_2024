...
const Table_Client = ({ data }: Props) => {
    ...
    async function deleteCliente(id:unknown){
        const confirmation = window.confirm('Are you sure you want to delete this client?')
        const id5 = +id
            if (confirmation) {
                try {
                    const api = new Api();
                    const response = await (await (api.deleteClien(id5))).data
                    // console.log(response)
    ...
    const filteredData = data.filter((dato) => {
        const matchByState = dato.state?.toLowerCase().includes(search.toLocaleLowerCase())
            ...
            const matchByFq = dato.bl?.toLowerCase().includes(search.toLocaleLowerCase())
            ...
            return Object.entries(checkboxState).some(([key, value]) => value && dato?.bl.toLowerCase() === key.toLowerCase()) && searchMatch;
          });
        ...
        <div className='shadow-md sm:rounded-lg'>
                ...
                <div className="flex">
                    {Object.entries(checkboxState).map(([key, value]) => (
                        <div key={key} className="flex items-center me-4">
                            ...
                            />
                            <label htmlFor="">BLACK LIST</label>
                        </div>
                    ))}
                </div>
            </div>
            <div className='overflow-auto bg-slate-300 flex items-center justify-between flex-column flex-wrap md:flex-row space-y-4 md:space-y-0 pb-4 bg-black dark:bg-slate-300 '>
                ...
                <thead className="text-xs text-black-300 uppercase bg-gray-500 dark:bg-gray-500 dark:text-black-400">
                        ...
                        <th scope="col" className="px-6 py-3">BLACK LIST</th>
                    ...
                    <tbody className="text-xs text-black-300 uppercase bg-slate-300 dark:bg-slate-200 dark:text-black-400">
                        {filteredData?.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).map((item) => (
                            <tr key={item.id} style={{ backgroundColor: item.bl === 'yes' ? 'pink' : 'inheret'}}>
                                ...
                                <td className="border-b border-slate-400 px-6 py-4">{item.bl}</td>
                                ...
                                <td className="border-b border-slate-400 px-6 py-9 flex">
                                    <>
                                        <button data-modal-target="authentication-modal" data-modal-toggle="authentication-modal"
                                            className="border-r-4 border-slate-500 font-medium text-blue-600 dark:text-blue-500 hover:underline " type="button"
                                    ...
                                    <button onClick={()=>deleteCliente(item.id)||null} className="border-l-4  font-medium text-red-800 dark:red-blue-800 hover:underline" >
